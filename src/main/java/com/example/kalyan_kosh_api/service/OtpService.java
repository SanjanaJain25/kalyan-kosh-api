...
public void sendOtp(String mobile) {
    // Check mobile exists in registered users
    ...
    OtpVerification otpEntity = otpRepo.findByMobile(mobile).orElse(new OtpVerification());
    otpEntity.setMobile(mobile);
    otpEntity.setOtpHash(hash);
    otpEntity.setExpiresAt(Instant.now().plusSeconds(300)); // 5 min
